    ...
    return(
        ...
        <div className="relative w-full h-full backdrop-blur-lg backdrop-filter flex flex-col justify-center items-center">
          <div className="absolute w-10/12  bg-slate-200 rounded-lg flex flex-col items-center p-5 gap-3 max-h-8/10 overflow-y-scroll">
            ...
            const [imageSrc, setImageSrc] = useState((brand['img_src'])  ? brand['img_src'] : '');
            ...
            return (<div key={uuidv4()} className='bg-gray-100 flex flex-col gap-3 justify-between items-center'>
                ...
                <input className='w-7 h-7' type="checkbox" defaultChecked={brand.active} onChange={()=>toggleActive(index)}/>
            </div>)}
            )}
            </div>
             <div>
